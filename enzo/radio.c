...
#define MAP_DIO0_LORA_CDDONE   0x80  // 10------

#define MAP_DIO1_LORA_RXTOUT   0x00  // --00----
#define MAP_DIO1_LORA_CDDETD   0x20  // --10----
...
static void configLoraModem () {
    sf_t sf = getSf(ENZO.rps);

#ifdef CFG_sx1276_radio
        u1_t mc1 = 0, mc2 = 0, mc3 = 0;

        switch (getBw(ENZO.rps)) {
        ...
        if (getIh(ENZO.rps)) {
            ...
            writeReg(LORARegPayloadLength, getIh(ENZO.rps)); // required length
        }
        // set ModemConfig1
        writeReg(LORARegModemConfig1, mc1);

        mc2 = (SX1272_MC2_SF7 + ((sf-1)<<4));
        if (getNocrc(ENZO.rps) == 0) {
            mc2 |= SX1276_MC2_RX_PAYLOAD_CRCON;
        }
        writeReg(LORARegModemConfig2, mc2);
        
        mc3 = SX1276_MC3_AGCAUTO;
        if ((sf == SF11 || sf == SF12) && getBw(ENZO.rps) == BW125) {
            mc3 |= SX1276_MC3_LOW_DATA_RATE_OPTIMIZE;
        }
        writeReg(LORARegModemConfig3, mc3);
#elif CFG_sx1272_radio
        u1_t mc1 = (getBw(ENZO.rps)<<6);
...
static void configChannel () {
    ...
    u8_t frf = ((u8_t)ENZO.freq << 19) / 32000000;
...
static void txfsk () {
    ...
    writeReg(FSKRegPayloadLength, ENZO.dataLen+1); // (insert length byte into payload))

    // download length byte and buffer to the radio FIFO
    writeReg(RegFifo, ENZO.dataLen);
    writeBuf(RegFifo, ENZO.frame, ENZO.dataLen);
...
static void txlora () {
    ...
    writeReg(LORARegPayloadLength, ENZO.dataLen);
...
// start transmitter (buf=ENZO.frame, len=ENZO.dataLen)
static void starttx () {
    ASSERT( (readReg(RegOpMode) & OPMODE_MASK) == OPMODE_SLEEP );
    if(getSf(ENZO.rps) == FSK) { // FSK modem
        txfsk();
    } else { // LoRa modem
        txlora();
    }
...
static const u1_t rxlorairqmask[] = {
    [RXMODE_SINGLE] = IRQ_LORA_RXDONE_MASK|IRQ_LORA_RXTOUT_MASK|IRQ_LORA_CRCERR_MASK|IRQ_LORA_HEADER_MASK,
    [RXMODE_SCAN]   = IRQ_LORA_RXDONE_MASK|IRQ_LORA_CRCERR_MASK|IRQ_LORA_HEADER_MASK,
    [RXMODE_RSSI]   = 0x00,
};

// start LoRa receiver (time=ENZO.rxtime, timeout=ENZO.rxsyms, result=ENZO.frame[ENZO.dataLen])
static void rxlora (u1_t rxmode) {
    ...
    // errata; optimized rx spurious response, bit 7 of 0x31 should be set to 0
    ...
    writeReg(LORARegSymbTimeoutLsb, ENZO.rxsyms);
    // set sync word
    writeReg(LORARegSyncWord, LORA_MAC_PREAMBLE);
    ...
    if (rxmode == RXMODE_SINGLE) { // single rx
        hal_waitUntil(ENZO.rxtime); // busy wait until exact rx time
...
static void rxfsk (u1_t rxmode) {
    ...
    writeReg(FSKRegRxTimeout2, 0xFF);//(ENZO.rxsyms+1)/2);
    // set bitrate
    writeReg(FSKRegBitrateMsb, 0x02); // 50kbps
    writeReg(FSKRegBitrateLsb, 0x80);
    // set frequency deviation
...
void radio_irq_handler (u1_t dio) {
    ...
    if( (readReg(RegOpMode) & OPMODE_LORA) != 0) { // LORA modem
        ...
        if( flags & IRQ_LORA_TXDONE_MASK ) {
            // save exact tx time
            ENZO.txend = now - us2osticks(43); // TXDONE FIXUP
        } else if( flags & IRQ_LORA_RXDONE_MASK ) {
            // save exact rx time
            if(getBw(ENZO.rps) == BW125) {
                now -= LORA_RXDONE_FIXUP[getSf(ENZO.rps)];
            }
            ENZO.rxtime = now;
            // read the PDU and inform the MAC that we received something
            ENZO.dataLen = (readReg(LORARegModemConfig1) & SX1272_MC1_IMPLICIT_HEADER_MODE_ON) ?
                readReg(LORARegPayloadLength) : readReg(LORARegRxNbBytes);
            // set FIFO read address pointer
            writeReg(LORARegFifoAddrPtr, readReg(LORARegFifoRxCurrentAddr)); 
            // now read the FIFO
            readBuf(RegFifo, ENZO.frame, ENZO.dataLen);
            // read rx quality parameters
            ENZO.snr  = readReg(LORARegPktSnrValue); // SNR [dB] * 4
            ENZO.rssi = readReg(LORARegPktRssiValue); // - 125 + 64; // RSSI [dBm] (-196...+63)
            ENZO.irq_flags = flags;
            ENZO.crcerr = !!(flags & IRQ_LORA_CRCERR_MASK);
            ENZO.validHeader = !!(flags & IRQ_LORA_HEADER_MASK); // Valid header
          ...
          // cad done
          ENZO.rxtime = now;
          ENZO.cad = !!(flags & IRQ_LORA_CDDETD_MASK);
          ENZO.dataLen = 0;
        }
        // mask all radio IRQs
        ...
        if( flags2 & IRQ_FSK2_PACKETSENT_MASK ) {
            ...
            ENZO.rxtime = now;
            // read the PDU and inform the MAC that we received something
            ENZO.dataLen = readReg(FSKRegPayloadLength);
            // now read the FIFO
            readBuf(RegFifo, ENZO.frame, ENZO.dataLen);
            // read rx quality parameters
            ENZO.snr  = 0; // determine snr
            ENZO.rssi = 0; // determine rssi
    ...
    os_setCallback(&ENZO.osjob, ENZO.osjob.func);
}

static void startcad() {
  ASSERT((readReg(RegOpMode) & OPMODE_MASK) == OPMODE_SLEEP);
  ASSERT(getSf(ENZO.rps) != FSK);
  // select LoRa modem (from sleep mode)
  opmodeLora();
  ASSERT((readReg(RegOpMode) & OPMODE_LORA) != 0);
  // enter standby mode (warm up))
  ...
  // configure DIO mapping DIO0=CadDone DIO1=CadDetected DIO2=NOP
  writeReg(RegDioMapping1, MAP_DIO0_LORA_CDDONE|MAP_DIO1_LORA_CDDETD|MAP_DIO2_LORA_NOP);
  // clear all radio IRQ flags
  writeReg(LORARegIrqFlags, 0xFF);
  // enable required radio IRQs
  ...
  // now instruct the radio to start the CAD process
  opmode(OPMODE_CAD);
}
...
void os_radio (u1_t mode) {
    ...
    switch (mode) {
      ...
      case RADIO_TX:
        // transmit frame now
        starttx(); // buf=ENZO.frame, len=ENZO.dataLen
        break;
      
      case RADIO_RX:
        // receive frame now (exactly at rxtime)
        startrx(RXMODE_SINGLE); // buf=ENZO.frame, time=ENZO.rxtime, timeout=ENZO.rxsyms
        break;

      case RADIO_RXON:
        // start scanning for beacon now
        startrx(RXMODE_SCAN); // buf=ENZO.frame
        break;
      case RADIO_CAD:
        startcad();
        break;
